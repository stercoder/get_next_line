...
/*   Updated: 2024/12/10 00:35:46 by ykhindou         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	read_next(int fd, char *buffer, char **str)
{
	int		r;
	char	*tmp;
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	line = get_line(&cont[fd]);
	while (!line && r > 0)
	{
		r = read_next(fd, buffer, &cont[fd]);
		if (r < 0)
			break ;
		line = get_line(&cont[fd]);
	}
	return (cleanup(&cont[fd], buffer, line, r));
}
